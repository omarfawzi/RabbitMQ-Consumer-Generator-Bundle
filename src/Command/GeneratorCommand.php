...
final class GeneratorCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while ($generateAnotherConsumer) {
            ...
            $consumer->setName($this->requiredQuestion($input, $output, new Question('Please Enter The Consumer Name : ')));
            $consumer->setQueueRoutingKeys($this->requiredQuestion($input, $output, new Question("Please Enter The Consumer Routing Keys separated by ',' : "), true));
            $consumer->setExchangeType($this->requiredQuestion($input, $output, new Question('Please Enter The Exchange Type : ')));
            $consumer->setExchangeName($this->requiredQuestion($input, $output, new Question('Please Enter The Exchange Name : ')));
            $consumer->setQueueName($this->requiredQuestion($input, $output, new Question('Please Enter The Queue Name : ')));
     ...
     * @return mixed
     */
    private function requiredQuestion(InputInterface $input, OutputInterface $output, $question, bool $arrayAnswer = false)
    {
        $helper = $this->getHelper('question');
        $result = null;
        if ($arrayAnswer && $question instanceof Question) {
            $question->setValidator(function ($answer) use ($output) : array {
